...
- **seed parity** — note-for-note agreement between the bundled catalogue and the
  seed data, so the six songs are identical online or offline

The catalogue holds exactly six public-domain nursery songs. The same six are
...
- **`songs`** — exported array of six `Song` objects: `twinkle` (Twinkle Twinkle
  Little Star), `mary` (Mary Had a Little Lamb), `hotcrossbuns` (Hot Cross Buns),
  `londonbridge` (London Bridge), `oldmacdonald` (Old MacDonald), and
  `rowyourboat` (Row, Row, Row Your Boat). Each id is a slug.
...
- Asserts `songs.length` is `6` with six distinct ids.
...
- **`Twinkle`**, **`Mary`**, **`HotCrossBuns`**, **`LondonBridge`**,
  **`OldMacDonald`**, **`RowYourBoat`** — one builder per song, encoded
  note-for-note identical to `frontend/src/songs/catalog.ts` per the file's own
  header comment.
...
|-------|--------------|-------------|
| `SL-L2-3` | `SL-L1-2, SL-L1-7` | The product shall bundle exactly six public-domain songs — Twinkle Twinkle Little Star, Mary Had a Little Lamb, Hot Cross Buns, London Bridge, Old MacDonald, and Row Row Row Your Boat — each with unique id and each satisfying the catalogue invariants. |
| `SL-L2-15` | `SL-L1-7` | The server's seed songs shall match the bundled catalogue note-for-note, so the six songs are identical whether served online or bundled offline. |
...
Family Server holds the same six songs as its seed data.

![C4 system context for the bundled catalogue](diagrams/c4-context.png)

### Containers

The six songs are compiled into the web app and, separately, seeded into the
...
`catalog.ts` supplies the six `Song` objects to the offline repository, while
`SeedData` writes the identical six into the family database through
`AppDbContext`.
...
With no network and no cache, the repository serves the six bundled songs
